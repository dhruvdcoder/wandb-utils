...
def get_args():
    ...
    parser.add_argument('-e', '--entity', required=True, help='Wandb entity (username or team)')
    parser.add_argument('-p', "--project", required=True, type=str, help="Wandb project (default:None)")
    parser.add_argument('-m', "--metric", required=True, type=str, help="Name of the metric to sort by (default:None)")
    parser.add_argument('-o', "--output_file", required=False, type=Path, help="File to which the run names will be written/appended."
                        " If not provided, the name will be printed on the console. (default:None)")
    parser.add_argument('-a', "--append", required=False, action="store_true", help="Set to append to file instead of overwritting (default:False)")
...
def main(args):
    ...
    logger.info(f"Following are the models:\n{df[['sweep', 'run', args.metric]]}")
    existing_runs = []
